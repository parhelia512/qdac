...
resourcestring
  ...
  SOutOfIntervalRange = '时间间隔类型不支持指定的值 %d (有效范围%d-%d)';

  SBadIntervalValue = '无效的时间间隔值:%s';

  SBadPgIntervalString = '无效的PostgreSQL时间间隔字符串:%s';

  SBadISOIntervalValue = '无效的ISO时间间隔字符串:%s';

  SBadOracleIntervalValue = '无效的Oracle时间间隔字符串:%s';

  SBadSQLIntervalString = '无效的SQL时间间隔字符串:%s';

  SIntervalOutOfRange = '时间间隔值超出了有效范围，约为-699051年4月~699050年7月';

  SBadTimeValue = '指定的时间值无效:%d-%d-%d %d:%d:%d.%d';

  SMonthOutOfRange = '指定的月份值 %d 超出有效范围';

  SDayOutOfRange = '指定的天数 %d 超出有效范围 (1-%d)';

  SHourOutOfRange = '指定的小时数 %d 超出有效范围(0-23)';

  SMinuteOutOfRange = '指定的分钟数 %d 超出有效范围(0-59)';

  SSecondOutOfRange = '指定的秒数 %d 超出有效范围(0-59)';

  SMSOutOfRange = '指定的毫秒数 %d 超出有效范围(0-599)';

  SMacroSecondOutOfRange = '指定的微秒数 %d 超出有效范围(0-999999)';

  SBadDateTimeString = '不支持的日期时间字符串值:%s';

  SBadPlanMask = '指定的计划掩码字符串 %s 无效';
...
begin
  ...
  if (MT > 8388607) or (MT < -8388608) then
...
begin
  ...
  if (M > 8388607) or (M < -8388608) then
...
begin
  ...
  Result := (Data and $FFFF3FE000000000) <> 0;
...
const
  ...
  WeekDayNames: array [1 .. 7] of QStringW = ('MON',
  ...
  MonthNames: array [1 .. 12] of QStringW = ('JAN',
      ...
      begin
        ...
        repeat
          ...
          begin
            ...
            if (V = 0) and (AIdx = tlpDayOfWeek) then
